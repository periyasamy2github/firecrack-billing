...
export const ThermalReceipt = ({ bill, tendered, changeDue, showQr, showMrpSaved }: ThermalReceiptProps) => {
  ...
  return (
    <div className={styles.sheet}>
      ...
      <Row label="Gross (MRP)" value={formatAmount(totals.gross)} />
      <Row label={`Item disc. ${totals.avgDiscountPct.toFixed(0)}%`} value={`-${formatAmount(totals.discount)}`} />
      ...
      {showMrpSaved && (
        <Typography className={styles.savedLine}>
          YOU SAVED {formatCurrency(totals.discount + totals.billDiscountAmount)}
        </Typography>
      )}
      {showQr && (
